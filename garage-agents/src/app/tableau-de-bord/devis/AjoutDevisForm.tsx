...
import { ajouterDevis, type AjoutDevisState } from "./actions";

const initialState: AjoutDevisState = {};
const inputCls =
...
export function AjoutDevisForm() {
  const [state, formAction, pending] = useActionState(ajouterDevis, initialState);
  ...
  return (
    <form ref={formRef} action={formAction} className="bg-surface border border-border rounded-lg p-5 flex flex-col gap-3.5">
      <h2 className="font-extrabold text-sm">Ajouter un devis</h2>
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-xs font-bold text-text-dim">Montant (€)</span>
          <input name="montant" type="number" min="0" step="0.01" required placeholder="1900" className={inputCls} />
        </label>
        <label className="flex flex-col gap-1.5">
